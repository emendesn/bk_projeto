...
/**************************************************************************
*** Programa: BKCOMA03      | Autor: Adilson do Prado  | Data: 10/04/2012 *
***************************************************************************
*** Descricao: Inclusao Benef�cios VT/VR/VA Pr�-Documento de Entrada e    *
***            Assist�ncia M�dica                                         *
***************************************************************************
*** Parametros:                                                           *
***************************************************************************
...
/**************************************************************************
*** Programa: MontaItemNF   | Autor:                   | Data: 10/04/2012 *
***************************************************************************
*** Descricao:                                                            *
***************************************************************************
*** Parametros:                                                           *
...
IF LEN(aItemLinha) == 0
   MSGSTOP("Itens da NF n�o encontrado verifique o arquivo de Lote","Aten��o")
   RETURN NIL
ENDIF
...
IF LEN(aItemCC) > 0
    ...
    IF (nValor - nTotal) > nMAXRAT
		MsgStop("Valor do rateio maior que valor maximo verifique Par�metros Tabela X5 - Z3, Valor Total para Rateio: "+STR((nValor - nTotal),10,2)+"   Valor M�ximo:"+STR(nMAXRAT,10,2), "Aten��o")
 	    RETURN NIL    
    ENDIF
	...
	IF Len(aSemCC) > 0
    	...
    	NEXT nX
		Aviso( "Aten��o", "Codigo Funcion�rio(s) n�o possui Centro de Custo: "+CHR(13)+CHR(10)+cCodFunc, { "OK" } )
	ENDIF

	
	IF TRANSFORM(nTotal,"@E 999,999,999.99") <> TRANSFORM(nValor,"@E 999,999,999.99")
	   MSGSTOP("Valor total da NF diferente do valor calculado favor verificar: "+TRANSFORM(nValor,"@E 999,999,999.99")+"     "+TRANSFORM(nTotal,"@E 999,999,999.99"),"Aten��o")
 	   RETURN NIL
	ENDIF
...
/**************************************************************************
*** Programa: ValidaNF      | Autor:                   | Data: 10/04/2012 *
***************************************************************************
*** Descricao:                                                            *
***************************************************************************
*** Parametros:                                                           *
...
IF EMPTY(cDoc) .OR. cDoc == '000000000'
	MsgStop("N�mero do Pr�-Documento de Entrada incorreto", "Aten��o")
	lOk:= .F.
    RETURN lOk 
Endif
   
IF EMPTY(cForn)
	MsgStop("N�mero do Fornecedor incorreto", "Aten��o")
	lOk:= .F.
    RETURN lOk 
Endif

IF EMPTY(cEspec)
	MsgStop("Esp�cie do Pr�-Documento de Entrada incorreto", "Aten��o")
	lOk:= .F.
    RETURN lOk 
Endif

IF EMPTY(cEspec)
	MsgStop("UF do Pr�-Documento de Entrada incorreto", "Aten��o")
	lOk:= .F.
    RETURN lOk 
Endif

IF EMPTY(cProduto)
	MsgStop("Produto do Pr�-Documento de Entrada incorreto", "Aten��o")
	lOk:= .F.
    RETURN lOk 
Endif

IF nValor <= 0
	MsgStop("Valor do Pr�-Documento de Entrada incorreto", "Aten��o")
	lOk:= .F.
    RETURN lOk 
Endif

IF EMPTY(cTipoNF)
	MsgStop("Tipo do Pr�-Documento de Entrada incorreto", "Aten��o")
	lOk:= .F.
    RETURN lOk 
Endif

IF EMPTY(cArq)  .And. SUBSTR(cTipoNF,1,2) <> "AS"
	MsgStop("Arquivo de importa��o de rateio do Pr�-Documento de Entrada incorreto", "Aten��o")
	lOk:= .F.
    RETURN lOk 
Endif
...
IF SF1->(dbSeek(xFilial("SF1")+cDoc+cSerie+cForn+cLoja+"N",.F.))
	MsgStop("Pr�-Documento de Entrada j� Existe", "Aten��o")
	lOk:= .F.
    RETURN lOk 
Endif
...
IF LEN(aArq) > 0 .And. SUBSTR(cTipoNF,1,2) <> "AS"
	...
	IF SUBSTR(cTipoNF,1,2) $ cNomeArq
	   MsgInfo("Aquivo Ok!!")
	ELSE
	    MsgStop("Arquivo de importa��o de rateio do Pr�-Documento de Entrada incorreto, verifique a exten��o do arquivo", "Aten��o")
		lOk:= .F.
    	RETURN lOk 
    ENDIF
Endif
...
/**************************************************************************
*** Programa: IncluiNF      | Autor:                   | Data: 10/04/2012 *
***************************************************************************
*** Descricao:                                                            *
***************************************************************************
*** Parametros: <aItemCC>                                                 *
...
*** Uso:                                                                  *
**************************************************************************/
Static Procedure IncluiNF(aItemCC)
...
Begin Transaction
	...
	IF lMsErroAuto
		...
		MsgStop("Problemas em Pr�-Documento de Entrada "+cDoc+"    "+cSerie+", informe o setor de T.I. ", "Aten��o")
	    MostraErro()
		DisarmTransaction()
		Return
	Else
		Msginfo(OemToAnsi("Pr�-Documento de Entrada incluido com sucesso! ")+cDoc+"    "+cSerie)
	EndIf
...
/**************************************************************************
*** Programa: GrvMVARQ      | Autor:                   | Data: 10/04/2012 *
***************************************************************************
*** Descricao:                                                            *
***************************************************************************
*** Parametros:                                                           *
...
/**************************************************************************
*** Programa: MontaPlaAS    | Autor:                   | Data: 10/04/2012 *
***************************************************************************
*** Descricao:                                                            *
***************************************************************************
*** Parametros:                                                           *
...
/**************************************************************************
*** Programa: ValidaAS      | Autor:                   | Data: 10/04/2012 *
***************************************************************************
*** Descricao:                                                            *
***************************************************************************
*** Parametros:                                                           *
...
IF VAL(SUBSTR(cPlano,1,6)) < 1 
	MsgStop("Plano BS Rubi incorreto"+cPlano, "Aten��o")
	lOk:= .F.
    RETURN lOk 
Endif
...
/**************************************************************************
*** Programa: MontaItemAS   | Autor:                   | Data: 10/04/2012 *
***************************************************************************
*** Descricao:                                                            *
***************************************************************************
*** Parametros: <cPlano>                                                  *
...
*** Uso:                                                                  *
**************************************************************************/
Static Procedure MontaItemAS(cPlano)
...
IF LEN(aItemAS) > 0
	...
	IF TRANSFORM(nTotPlan,"@E 999,999,999.99") <> TRANSFORM(nValor,"@E 999,999,999.99")
	   MSGSTOP("Valor total da NF diferente do valor total do plano: "+TRANSFORM(nValor,"@E 999,999,999.99")+"     "+TRANSFORM(nTotPlan,"@E 999,999,999.99"),"Aten��o")
	ENDIF
...
/**************************************************************************
*** Programa: ValidaBS      | Autor:                   | Data: 10/04/2012 *
***************************************************************************
*** Descricao:                                                            *
***************************************************************************
*** Parametros: <cPlano>                                                  *
...
IF TRANSFORM(nTotPlan,"@E 999,999,999.99") <> TRANSFORM(nValor,"@E 999,999,999.99")
	MSGSTOP("Valor total da NF diferente do valor total do plano: "+TRANSFORM(nTotPlan,"@E 999,999,999.99")+"     "+TRANSFORM(nValor,"@E 999,999,999.99"),"Aten��o")
  	ok := .F.
ENDIF
...
/**************************************************************************
*** Programa: ItemCusto     | Autor:                   | Data: 10/04/2012 *
***************************************************************************
*** Descricao:                                                            *
***************************************************************************
*** Parametros: <cQTPACC>                                                 *
...
/**************************************************************************
*** Programa: ItemDep       | Autor:                   | Data: 10/04/2012 *
***************************************************************************
*** Descricao:                                                            *
***************************************************************************
*** Parametros:  <cPlano> -                                               *
...
/**************************************************************************
*** Programa: MontaBS       | Autor:                   | Data: 10/04/2012 *
***************************************************************************
*** Descricao:                                                            *
***************************************************************************
*** Parametros: <aItemAS> -                                               *
...
*** Uso:                                                                  *
**************************************************************************/
Static PROCEDURE MontaBS(aItemAS)
Local nSnd  := 15,nTLin := 15
Local oDlg01,aButtons := {}
LOCAL nX
LOCAL lOk := .F.
...
/**************************************************************************
*** Programa: ValidaInNF    | Autor:                   | Data: 10/04/2012 *
***************************************************************************
*** Descricao:                                                            *
***************************************************************************
*** Parametros: <nTotal> -                                                *
