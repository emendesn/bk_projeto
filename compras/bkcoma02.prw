...
/**************************************************************************
*** Programa: BKCOMA02      | Autor: Marcos B. Abrahao | Data: 29/09/2009 *
***************************************************************************
*** Descricao: Selecao de NF - Integracao VT/VR                           *
***                                                                       *
*** Integrado com o aplicativo :                                          *
***                     http://intranet.bkinformatica.com/WebIntegraRubi/ *
***                                                                       *
*** Tratamento do campo campo Z2_STATUS pelos sistemas:                   *
***                                                                       *
*** " "- Gerado ou alterado pelo RH: financeiro pode importar ou RH pode  *
***      manipular                                                        *
*** "X"- Em selecao, na tela do financeiro: RH n�o pode manipular         *
*** "S"- Titulos gerados no financeiro: financeiro pode excluir,RH n�o    *
***      pode manipular                                                   *
*** "D"- Titulos excluidos no financeiro: RH pode manipular               *
***                                                                       *
...
dbGoTop()
IF BOF() .OR. EOF()
	MsgStop("N�o ha VT/VR gerados", "Aten��o")
	RestArea(aAreaIni)
	Return
ENDIF

// Verificar se h� processos de integra��o em andamento
...
IF nStatusX > 0
    MsgStop("Integra��o j� iniciada por outra se��o", "Aten��o")
	IF __cUserId <> "000000"
	   MsgStop("Verifique se h� algum usuario processando a integra��o, caso contr�rio comunique o administrador do sistema", "Aten��o")
    ELSE
...
If Empty(aCtrId)
	MsgStop("N�o h� NFs dispon�veis", "Aten��o")
	RestArea(aAreaIni)
	Return
EndIf
...
DEFINE MSDIALOG oDlg TITLE "Sele��o de NF - Integra��o VT/VR" FROM 000,000 TO 400,630 PIXEL 
...
@ 015, 005 LISTBOX oListID FIELDS HEADER "","S�rie","NF","Fornecedor","Loja","Mes","Total R$" SIZE 310,170 OF oPanelLeft PIXEL 
...
/**************************************************************************
*** Programa: ConfTit       | Autor:                   | Data: 02/09/2009 *
***************************************************************************
*** Descricao:                                                            *
***************************************************************************
*** Parametros: <aTitGer> -                                               *
...
/**************************************************************************
*** Programa: GravaSe2      | Autor:                   | Data: 02/09/2009 *
***************************************************************************
*** Descricao:                                                            *
***************************************************************************
*** Parametros: <aTitGer> -                                               *
...
*** Uso:                                                                  *
**************************************************************************/
Static Procedure GravaSe2(aTitGer)
...
If !lOk
	MsgStop("Titulo "+cKey1+" j� existente, informe o setor de T.I.", "Aten��o")
	Return
EndIf
*/
...
For nI := 1 TO LEN(aTitGer)
	...
	If lMsErroAuto
	   MsgStop("Problemas na gera��o do titulo "+cKey2+", informe o setor de T.I.", "Aten��o")
	   Return
	EndIf
...
/**************************************************************************
*** Programa: ValidaMrk     | Autor:                   | Data: 02/09/2009 *
***************************************************************************
*** Descricao:                                                            *
***************************************************************************
*** Parametros:  <lRet> -                                                 *
...
IF CTOD(cPgto) >= dDataBase
   lRet := !lRet
ELSE
   MsgStop("Data de pgto deste lote � inferior a data base do sistema", "Aten��o")
   lRet := .F.
ENDIF   
Return lRet
