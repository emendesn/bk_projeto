...
/**************************************************************************
*** Programa: BKCOMR03       | Autor: Manuela /Marcos  | Data: 15/01/2008 *
***************************************************************************
*** Descricao: Protocolo NE                                               *
***************************************************************************
*** Parametros:                                                           *
***************************************************************************
*** Retorno: <Nil>                                                        *
...
Local cDesc3         := "Protocolo NE - Proqualit"
...
/**************************************************************************
*** Programa: RunReport      | Autor:                  | Data: 11/01/2008 *
***************************************************************************
...
Local nCount
Local cStaf,lStaf,cSuper,aUser,cMDiretoria,cFiltro := ""
...
IF __cUserId <> "000000" // Administrador
   ...
   //lAClas := MsgBox("Filtra os Doc a Classificar", "M103BROW", "YESNO")
   DBCLEARFILTER() 
   PswOrder(1) 
   PswSeek(__CUSERID) 
   aUser  := PswRet(1)
...
//Ajuste nova rotina a antiga n�o funciona na nova lib MDI
   ...
   // Se o usuario pertence ao grupo Administradores ou Master Diretoria : n�o filtrar
    IF ASCAN(aUser[1,10],"000000") = 0 .AND. !lMDiretoria
       IF !lStaf .OR. EMPTY(cSuper)
       	  IF EMPTY(cSuper) .AND. __cUserId $ cGerGestao
          	cFiltro := " AND (F1_XXUSER = '"+__cUserId+"' OR F1_XXUSERS = '"+__cUserId+"' OR F1_XXUSER = '      ' OR F1_XXUSERS = '000175') "
       	  ELSE 
          	cFiltro := " AND (F1_XXUSER = '"+__cUserId+"' OR F1_XXUSERS = '"+__cUserId+"' OR F1_XXUSER = '      ') "
          ENDIF
       ELSE
       	  IF Staf .AND. cSuper $ cGerGestao
          	cFiltro := " AND (F1_XXUSER = '"+__cUserId+"' OR F1_XXUSERS = '"+__cUserId+"' OR F1_XXUSER = '      ' OR F1_XXUSERS = '000175' OR "
...
nomeprog := "BKCOMR03/"+TRIM(SUBSTR(cUsuario,7,15))
...
/**************************************************************************
*** Programa: ValidPerg      | Autor:                  | Data: 11/01/2008 *
***************************************************************************
*** Descricao:                                                            *
***************************************************************************
*** Parametros: <cPerg> -                                                 *
